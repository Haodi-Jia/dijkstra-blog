...
import { WechatCopyButton } from '@/components/layout/wechat-copy-button'
  ...
  return (
    <aside className={cn('sticky top-0 flex h-screen min-w-0 flex-col py-[26px] pr-2.5 pb-7 pl-[151px] max-[1100px]:pr-3 max-[1100px]:pl-14 max-[760px]:hidden', className)}>
      ...
      <div className="mt-[clamp(114px,13vh,146px)]">
        <Link aria-label={content.site.labels.backHome} href="/">
          <Avatar className="size-24 bg-[#283041] shadow-[0_15px_38px_rgba(32,39,55,0.14)] after:hidden dark:shadow-[0_15px_38px_rgba(0,0,0,0.3)]">
            <AvatarImage alt={`${siteConfig.name} ${content.site.labels.avatar}`} src={siteConfig.avatar} />
          </Avatar>
        </Link>
        ...
        {profile === 'full' && (
          <>
            <p className="mt-[13px] text-lg leading-[1.8] text-muted-foreground">{content.site.description}</p>
            <div aria-label={content.site.labels.socialLinks} className="mt-[27px] flex gap-5 text-muted-foreground/75">
              ...
              <a aria-label={content.site.labels.email} className="transition-[color,transform] duration-200 hover:-translate-y-0.5 hover:text-foreground" href={`mailto:${siteConfig.social.email}`}>{icons.mail}</a>
              <WechatCopyButton wechat={siteConfig.social.wechat} />
            </div>
          </>
        )}
      </div>
