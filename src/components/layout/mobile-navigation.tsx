...
export function MobileNavigation({ activeHref = '/' }: { activeHref?: string | null }) {
  return (
    <header className="sticky top-0 z-20 hidden h-16 grid-cols-[38px_38px_minmax(0,1fr)_40px_40px] items-center gap-2.5 border-b bg-background/96 px-4 backdrop-blur-[14px] max-[760px]:grid max-[380px]:grid-cols-[34px_34px_minmax(0,1fr)_36px_36px] max-[380px]:gap-[7px] max-[380px]:px-3">
      <Sheet>
        <SheetTrigger render={<Button aria-label={content.site.labels.openNavigation} className="size-[38px] p-[5px] max-[380px]:size-[34px]" size="icon" variant="ghost" />}>
          {icons.menu}
        </SheetTrigger>
        <SheetContent closeLabel={content.site.labels.close} className="border-0 bg-popover px-[26px] py-6 shadow-[16px_0_50px_rgba(20,25,35,0.12)] data-[side=left]:w-[min(78vw,320px)] data-[side=left]:sm:max-w-[320px] dark:shadow-[16px_0_50px_rgba(0,0,0,0.35)]" side="left">
          <SheetTitle className="sr-only">{content.site.labels.mainNavigation}</SheetTitle>
          <nav aria-label={content.site.labels.mobileNavigation} className="mt-[72px] grid gap-[7px]">
            {navigation.map((item) => (
              <Button
      ...
      <Button aria-label={content.site.labels.searchArticles} className="size-10 p-[7px] text-muted-foreground max-[380px]:size-9" size="icon" variant="ghost">{icons.search}</Button>
