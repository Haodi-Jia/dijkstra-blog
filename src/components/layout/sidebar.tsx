...
export function Sidebar({ activeHref = '/' }: { activeHref?: string | null }) {
  return (
    <DesktopSidebar>
      <nav aria-label="主导航" className="mt-[clamp(46px,7vh,70px)] grid gap-[5px]">
        {navigation.map((item) => (
          <Button
      ...
      <p className="mt-auto text-xs tracking-[0.05em] text-muted-foreground/60">{siteConfig.copyright}</p>
    </DesktopSidebar>
  )
}
